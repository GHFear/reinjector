// RE::LLUSORY DLL INJECTOR 0.2.2 | A Dll Injector By GHFear.
// Uses C and Win32 API.
...
LRESULT CALLBACK InjectorMainWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

INT APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
    WNDCLASS wc = {0};
    wc.lpfnWndProc = InjectorMainWindowProc;
    wc.hInstance = hInstance;
    wc.lpszClassName = _T("InjectorWindowClass");
    wc.hbrBackground = CreateSolidBrush(RGB(64, 64, 64));
    wc.hIcon = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_ICON1));

    RegisterClass(&wc);

    HWND hwnd = CreateWindowEx(0, wc.lpszClassName, _T("RE::LLUSORY DLL INJECTOR 0.2.2"),
...
INT set_injection_type(DWORD selected_process_id,  const wchar_t* wide_dll_file_path, const wchar_t* wide_exe_file_path)
{
    ...
    else if (InjectionType == 1)
    {
        ...
        {
            printf("Error converting shellcode text to shellcode byte array.");
